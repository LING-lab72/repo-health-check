...
系统 SHALL 提供 `_test_github_direct()` 函数，使用 `git -c http.proxy="" -c https.proxy="" ls-remote --heads` 测试 GitHub 是否可直连。`-c` 配置参数 SHALL 位于 git 子命令之前。超时 SHALL 为 15 秒。
...
系统 SHALL 通过 `git -c http.proxy=... -c https.proxy=... clone ...` 命令行 flag 传递代理参数给 git clone 命令，不依赖 `git config --global` 全局配置。无代理时 SHALL 显式传递空值以覆盖可能存在的全局代理配置。`-c` 参数 SHALL 位于 `clone` 子命令之前。

#### Scenario: 使用代理克隆

- **WHEN** 需要通过代理克隆且代理地址为 `http://127.0.0.1:7890`
- **THEN** git 命令形如 `git -c http.proxy=http://127.0.0.1:7890 -c https.proxy=http://127.0.0.1:7890 clone ...`

#### Scenario: 不使用代理克隆

- **WHEN** 直连成功无需代理
- **THEN** git 命令形如 `git -c http.proxy= -c https.proxy= clone ...`（空值覆盖全局配置）
