...
系统 SHALL 在写入缓存和读取缓存时使用深拷贝隔离嵌套结构，防止调用方修改返回对象中的 dimensions、issues 或 ai_diagnosis 后污染缓存。

#### Scenario: 嵌套列表不被外部修改
- **WHEN** 调用方获取缓存结果并修改返回对象中的 `dimensions[0].score`
- **THEN** 后续再次读取同一缓存 key 时，缓存中的原始 score 不受影响
